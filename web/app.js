(() => {
    ...
    const storyItems = document.getElementById("story-items");
    ...
    function renderJob(job) {
        state.job = job; storyItems.innerHTML = "";
        resultsSummary.textContent = `${job.item_count} عنصر • ${job.video_count} فيديو • ${job.image_count} صورة`;
        downloadAllButton.href = `/api/jobs/${job.id}/download-all`;
        mergeButton.hidden = job.video_count < 2;
        job.items.forEach((item) => {
            ...
            node.querySelector(".item-title").textContent = item.title || `عنصر القصة ${item.order}`;
            node.querySelector(".item-meta").textContent = `${itemLabel(item)} • ${String(item.extension || "").toUpperCase()}`;
